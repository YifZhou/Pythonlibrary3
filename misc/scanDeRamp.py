...
def ackBarCorrector1(t, weights, counts, p, expTime, include_transit=False):
...
def ackBarCorrector2(t, weights, counts, p, expTime, scanDirect):
    """correct the ackbar model for one directional scan observations
    ...
    for i in range(nLC):
        if twoDirect:
            ramp, crates, slope = ackBarCorrector2(
                time, weights, LCmatrix[i, :], p, expTime, scanDirect)
        else:
            ramp, crates, slope = ackBarCorrector2(
                time, weights, LCmatrix[i, :], p, expTime)
        rampMat[i, :] = ramp
        slopeMat[i, :] = slope
...
def scanDeRamp(p0,
    ...
    rampMat, slopeMat = visit_deRamp(
        p0, time, mat_binned, err_binned, weights, expTime,
        scanDirect)
    ...
    return LCmatrix_deramp, Errmatrix_deramp
